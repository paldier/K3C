...
#define DMA_PACKET_SIZE                         1696    //  ((1518 + 8 <2 VLAN> + 62 <PPE FW> + 8 <SW Header>) + 31) & ~31
#else
...
static uint32_t wlan_pltfrm_ppa_offload_get_wlanm_base(void)
{
    ...
    printk("%s: wlanm address is 0x%x\n",__func__,iMem);
    g_wlan_membase = iMem;
    return iMem;
}
#endif
