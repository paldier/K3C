...
wps_external_done()
{
	...
	do
		...
		then
			...
			case "$fapi_rpc" in
			"false")
				[ "$enable_log" = "1" ] && echo "UGW, exec ubus_command=$ubus_command" > /dev/console
				# TEST:
				[ "$test_only" = "1" ] && exit
				# TEST END
			...
			;;
			"true")
				[ "$enable_log" = "1" ] && echo "Call convert_ugw_to_fapicli with ubus_command=$ubus_command" > /dev/console
				ubus_command_fapi=`convert_ugw_to_fapicli $ubus_command`
				ubus_command_fapi=`echo $(ubus_command_fapi//\"/)`
				[ "$enable_log" = "1" ] && echo "exec build_wlan_notification" > /dev/console
