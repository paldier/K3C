...
mtlk_hostapd_setup_base() {
	...
	[ -n "$ieee80211n" ] && {
		...
		mtlk_add_capabilities ht_capab_flags $ht_cap_mask \
			LDPC:0x1::$ldpc \
			GF:0x10::$greenfield \
...
#waiting for wlan0 wlan2 up 4.5s-5.5s
	...
	has_ap=
	hostapd_ctrl=
	for_each_interface "ap" mtlk_check_ap

	rm -f "$hostapd_conf_file"
	[ -n "$has_ap" ] && mtlk_hostapd_setup_base "$phy"

	for_each_interface "sta adhoc monitor" mtlk_prepare_vif
