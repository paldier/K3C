...
echo ""
echo ""
echo "Press Enter to execute step 11 for AP2"
read -p "Execute step 11 for AP2: " exec_step11

echo "Executing: hostapd_cli -i${interface_name} bss_tm_req $sta_addr pref=1 neighbor=${neighbor_ap_addr},10,81,36,9,20 mbo=4:50:-1 disassoc_imminent=1 disassoc_timer=100"
hostapd_cli -i${interface_name} bss_tm_req $sta_addr pref=1 neighbor=${neighbor_ap_addr},10,81,36,9,20 mbo=4:50:-1 disassoc_imminent=1 disassoc_timer=100
